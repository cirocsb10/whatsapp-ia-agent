...
2. `catalog/page.tsx` já quebrado; telas menores (orders, support, analytics) ainda não — impacto baixo, nenhuma delas está perto do tamanho que motivou o F5 originalmente.

**E2E além do smoke (§3.11) — feito, mas só local:** em vez de orquestrar postgres/redis/rabbitmq/api/channel-service/ai-orchestrator do zero no CI (custo/flakiness altos para o ganho), `apps/web/tests/e2e-full/` cobre login real → navegação cacheada → mensagem inbound chegando via socket (webhook assinado com HMAC, simulando a Meta) → handoff, rodando contra a stack local já em pé (`pnpm docker:up && pnpm dev`). Script `pnpm test:e2e:full` em `apps/web`, documentado em `tests/e2e-full/README.md`. **Não roda no CI** e **não foi executado de ponta a ponta nesta sessão** — o sandbox não tinha Docker Desktop ativo (só um Postgres nativo acessível, sem Redis/RabbitMQ), então o código foi validado por typecheck e por espelhar a mesma lógica de HMAC/auth de produção, mas precisa ser rodado manualmente contra uma stack local real antes de confiar nele.
